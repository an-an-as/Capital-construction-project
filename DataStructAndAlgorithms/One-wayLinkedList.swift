/**
 ...
 + 链表允许插入和删除表上任意位置上的节点，但是不允许随即存取。链表有很多种不同的类型：单向链表、双向链表及循环链表。
...
private enum ListNode<Element> {
    case end
    indirect case node (Element, ListNode)
    func cons(_ newElement: Element) -> ListNode {
...
public struct List<Element>: Collection {
    ...
    public func index(after listIndex: ListIndex<Element>) -> ListIndex<Element> {
        switch listIndex.node {
        case .end: fatalError()
        case .node(_, let next):
            return ListIndex(node: next, tag: listIndex.tag - 1)
        }
    }
    public subscript (pos: Index) -> Element {
        switch pos.node {
        case .end: fatalError()
        case .node(let value, _): return value
        }
    }
    public subscript (bounds: Range<Index>) -> List {
        return List(startIndex: bounds.upperBound, endIndex: bounds.lowerBound)
    }
}
extension List {
    @discardableResult
    mutating func pop() -> ListIndex<Element> {
        let current = startIndex
        startIndex = index(after: startIndex)
        return current
    }
    mutating func push(_ newElement: Element) {
        startIndex = ListIndex(node: startIndex.node.cons(newElement), tag: startIndex.tag + 1)
    }
}
...
extension List: ExpressibleByArrayLiteral {
    public init(arrayLiteral elements: Element...) {
        self.init(startIndex: ListIndex(node: elements.reduce(.end, { $0.cons($1) }),
                                        tag: elements.count), endIndex: ListIndex(node: .end, tag: 0))
    }
}
extension List: CustomStringConvertible {
    public var description: String {
        return self.reversed().map { "List: \($0)" }.joined(separator: "\n")
    }
}
