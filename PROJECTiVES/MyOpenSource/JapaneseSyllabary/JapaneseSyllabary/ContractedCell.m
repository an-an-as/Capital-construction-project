...
-(void)dealloc{
    ...
    self.cureentBtn=nil;
    self.gradient=nil;
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"cellBtn3" object:nil];
}
-(NSArray *)playerArray{
    if (!_playerArray) {
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self=[super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        [self creatNewButtonWithCount:3];
...
-(void)basicButtonClick:(UIButton*) button{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setBtnState:) name:@"cellBtn3" object:nil];
    NSDictionary*btn=@{@"button3":button};
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cellBtn3" object:nil userInfo:btn];
    
    NSString* str=self.playerArray[button.tag];
    NSString* str4=self.wordArray[button.tag];
    
    NSDictionary* dict=@{@"contracted":str4};
    [[NSNotificationCenter defaultCenter] postNotificationName:@"kanaWord3" object:nil userInfo:dict];
...
-(void)setBtnState:(NSNotification*)notification{
    UIButton*btn=notification.userInfo[@"button3"];
